...
const TEST_FILES_DIR := "res://addons/marshal_maker_tests/file_scouts/recursive_file_scout_test_files/"
...
func _filenames_to_fullpaths(filenames: Array[String]) -> Array[String]:
	...
	for filename in filenames:
		...
		var fullpath := MMPathLib.join(TEST_FILES_DIR, filename_w_suffix)
...
func _dirnames_to_fullpaths(dirnames: Array[String]) -> Array[String]:
	...
	for dirname in dirnames:
		var fullpath := MMPathLib.join(TEST_FILES_DIR, dirname)
		fullpaths.append(fullpath)
	return fullpaths

func _get_actual(ignored_paths: Array[String]) -> Array[String]:
	return MMRecursiveFileScout.new(TEST_FILES_DIR, ignored_paths).get_filepaths()
