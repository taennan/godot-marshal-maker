...
func _load_config() -> void:
	var config := MarshalMakerConfig.plugin_config()
	if not config:
		_logs.add_log("ERROR: No plugin config found at " + MarshalMakerConfig.PLUGIN_CONFIG_PATH)
		return
...
func _generate_files() -> void:
	var runner := MMWriterRunner.new(_directories.in_dir(), _directories.out_dir())
...
func _on_runner_marked_file_found(filepath: String) -> void:
	_logs.add_log("Found marshal_maker file: " + filepath)
...
func _update_config() -> void:
	var config := MarshalMakerConfig.new()
