...
static func vector2_from_array(array: Array[float]) -> Vector2:
	const default_value := 0.0
	var x: float = MMArrLib.get_at(array, 0, default_value)
	var y: float = MMArrLib.get_at(array, 1, default_value)
	return Vector2(x, y)
...
static func vector3_from_array(array: Array[float]) -> Vector3:
	...
	var x: float = MMArrLib.get_at(array, 0, default_value)
	var y: float = MMArrLib.get_at(array, 1, default_value)
	var z: float = MMArrLib.get_at(array, 2, default_value)
	return Vector3(x, y, z)
